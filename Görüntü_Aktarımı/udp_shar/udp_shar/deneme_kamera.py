...
receiver_ip = '192.168.0.51'  # Otomatik olarak kullanılacak (tüm arayüzler)
receiver_port = 9999
...
def send_request_periodically():
    while True:
        try:
            sock.sendto(b"REQUEST", (sender_ip, sender_port))
            print("REQUEST gönderildi.")
        except Exception as e:
            print(f"REQUEST gönderme hatası: {e}")
        time.sleep(5)

# Arka planda periyodik REQUEST gönderen thread başlat
threading.Thread(target=send_request_periodically, daemon=True).start()
başla=time.time()
while True:
...
print(f"Kod {bitir - başla:.2f} saniye sürdü")
