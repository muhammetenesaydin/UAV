...
class KamikazeUAVController:
    def __init__(self, config_file="config.py"):
        ...
        self.mav = mavutil.mavlink_connection('udp:127.0.0.1:14700')
        ...
        # QR fotoğraflarının kaydedileceği klasörü tanımla
        self.qr_photos_dir = "qr_photos"
        # Klasörün var olup olmadığını kontrol et ve yoksa oluştur
        if not os.path.exists(self.qr_photos_dir):
            os.makedirs(self.qr_photos_dir)
            ...
            while time.time() - start_time < timeout:
                ...
                try:
                    ...
                    if (time.time() - start_time) % 3 < 0.1:
                        debug_filename = os.path.join(self.qr_photos_dir, f"qr_scan_{int(time.time())}.jpg")
                    ...
                    if decoded:
                        ...
                        qr_image_filename = os.path.join(self.qr_photos_dir, f"qr_detected_{int(time.time())}.jpg")
                        cv2.imwrite(qr_image_filename, frame)
                        print(f"QR kod görüntüsü kaydedildi: {qr_image_filename}")
            ...
            if self.vehicle_state['is_flying']:
                ...
                if self.change_mode('FBWA'):
                    ...
                    while time.time() - yunus_baslangic < yunus_suresi:
                        ...
                        if self.vehicle_state['is_flying'] and self.vehicle_state['altitude'] > self.config["ucus_parametreleri"]["target_altitude"]:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print(f"Özel konfigürasyon dosyası kullanılıyor: {config_file}")
        controller = KamikazeUAVController(config_file)
    else:
        print("Varsayılan konfigürasyon dosyası kullanılıyor: config.py")
        controller = KamikazeUAVController()
    
    # Dalış görevini başlat
